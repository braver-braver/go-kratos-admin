...
func newSysRole(db *gorm.DB, opts ...gen.DOOption) sysRole {
	...
	_sysRole.SortID = field.NewInt32(tableName, "sort_id")
	_sysRole.Menu = field.NewString(tableName, "menus")
	_sysRole.API = field.NewString(tableName, "apis")
...
type sysRole struct {
	...
	Menu      field.String // 分配的菜单列表
	API       field.String // 分配的API列表
	ParentID  field.Int32  // 上一层角色ID
...
func (s *sysRole) updateTableName(table string) *sysRole {
	...
	s.Menu = field.NewString(table, "menus")
	s.API = field.NewString(table, "apis")
...
func (s *sysRole) fillFieldMap() {
	...
	s.fieldMap["menus"] = s.Menu
	s.fieldMap["apis"] = s.API
