...
type sysMenu struct {
	...
	Type      field.String // 菜单类型 FOLDER: 目录 MENU: 菜单 BUTTON: 按钮
	Path      field.String // 路径,当其类型为'按钮'的时候对应的数据操作名,例如:/user.service.v1.UserService/Login
	Redirect  field.String // 重定向地址
	Alias_    field.String // 路由别名
	Name      field.String // 路由命名，然后我们可以使用 name 而不是 path 来传递 to 属性给 <router-link>。
	Component field.String // 前端页面组件
	Metum     field.String // 前端页面组件
	ParentID  field.Int32  // 上一层菜单ID
...
func (s *sysMenu) updateTableName(table string) *sysMenu {
	...
	s.Metum = field.NewString(table, "meta")
